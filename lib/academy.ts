...
export const MODULES: Module[] = [
  {
    slug: "ingredient-science", section: "nutrition", title: "Ingredient Science — teach it, within the lines", estMin: 12,
    summary: "What every ingredient is, why we chose it, and the exact line between education and a claim you can't make.",
    whyItMatters: "Guests ask 'what's good for me?' every shift. The brand lives or dies on how you answer: educate them honestly and you earn trust for life; overpromise and you put the whole business a warning letter away. This lesson is the difference between confident and reckless.",
    objectives: ["Explain what each ingredient IS and one sourced fact about it", "State the hard line — composition facts OK, disease/detox/allergen claims never", "Handle the 'is it healthier / does it detox' question compliantly"],
    founderInsight: "We use real goat milk and Ceylon cinnamon because they're better, and we can say exactly why. What we will never do is promise a drink fixes someone — bodies don't work that way, and pretending otherwise is the opposite of Pure Signal. Tell the truth about what's in the cup; that's more than enough.",
    body: [
      { h: "Cold extraction", p: "Steeped slow and cold instead of rushed with heat — many people find it smoother and less bitter. Say that. Don't call it 'less acidic / easier on your stomach' — the acidity science is actually mixed." },
      { h: "A2 goat milk", p: "Naturally an A2 milk (no BCM-7 peptide), smaller softer curds, lower in lactose than cow milk. Say 'naturally A2, some find it easier to digest, lower in lactose.' Never 'lactose-free' or 'safe for a milk allergy' — it's still dairy." },
      { h: "Maple syrup", p: "Unrefined tree sap, keeps trace minerals (manganese, zinc), lower glycemic index than table sugar (~54 vs ~65). A better-choice sweetener — never 'safe for diabetics' or 'won't spike your blood sugar.' It's still sugar." },
      { h: "Sea salt & coconut water", p: "Sea salt gives sodium, the primary hydration electrolyte. Coconut water is naturally rich in potassium — a whole-food electrolyte source. Describe the electrolytes; don't claim 'most hydrating' or medical rehydration." },
      { h: "Cacao nibs, Ceylon cinnamon, cardamom", p: "Cacao = whole chocolate, magnesium + flavanols + theobromine (a gentler lift than caffeine). Ceylon is 'true cinnamon,' naturally low in coumarin unlike cassia. Cardamom is for aroma. Facts about the ingredient — never 'lowers blood sugar' or 'reduces inflammation.'" },
      { h: "Bone broth & nitro", p: "Broth is a whole-food source of protein and collagen with amino acids and minerals — never 'heals your gut.' Nitro is cold brew + nitrogen for a creamy pour, nothing added — a texture story, not a health story." },
      { h: "The 'does it detox' answer", p: "It's clean because of WHAT'S IN IT — whole foods, nothing artificial, no refined sugar, no seed oils, no synthetic additives — not because of anything it does to your body. Never say 'detox.' Never call seed oils or additives 'harmful' — that's a formulation choice, not a health verdict, and mainstream science doesn't back the verdict." },
    ],
    mistakes: ["Saying 'lactose-free' about goat milk (it isn't)", "Telling a diabetic maple syrup is 'safe' (it's still sugar)", "Using the word 'detox' or claiming a drink 'flushes toxins'", "Calling seed oils 'harmful' — say 'we don't use them,' never the verdict", "Quoting a specific caffeine or nutrition number we haven't verified"],
    quiz: [
      { q: "A guest asks: 'Is your cold brew less acidic so it won't bother my stomach?'", options: ["'Yes, 67% less acid, much gentler'", "'Many find it smoother and less bitter; on acidity the science is mixed — if that's a stomach concern, worth asking your doctor'", "'Yes, it's great for acid reflux'"], correct: 1, why: "We can describe taste and note the evidence is mixed; we cannot promise it's easier on the stomach — that's a health claim." },
      { q: "What must you NEVER call goat milk?", options: ["An A2 milk", "Lower in lactose than cow milk", "Lactose-free or safe for a milk allergy"], correct: 2, why: "Goat milk is lower in lactose, not free of it, and is not safe for a diagnosed milk allergy." },
      { q: "Why do we use Ceylon cinnamon — and is that a health claim?", options: ["It lowers blood sugar (a health benefit)", "It's 'true cinnamon,' naturally low in coumarin — a quality choice, not a health claim", "It cures inflammation"], correct: 1, why: "The low-coumarin fact is a real composition/quality point; benefit claims like blood sugar are off-limits." },
      { q: "A guest asks if a drink 'detoxes' them. Best answer?", options: ["'Yes, it flushes toxins and gives clean energy'", "'It's clean because of what's in it — whole foods, nothing artificial — not because of anything it does to your body'", "'It doesn't cost your body energy to detox'"], correct: 1, why: "'Detox' and 'doesn't burden your body' are unsupported metabolic claims; the composition statement is true and defensible." },
      { q: "Can we tell a diabetic our maple syrup is safe for them?", options: ["Yes — it's low glycemic", "No — we can say lower GI and unrefined with trace minerals, but never 'safe for diabetics'; it's still sugar", "Yes — it's natural so it's fine"], correct: 1, why: "GI and mineral facts are citable; medical-safety and blood-sugar claims are prohibited and untrue." },
      { q: "How should we talk about seed oils?", options: ["'They're inflammatory and bad for you, so we skip them'", "'We don't use them' — a formulation choice, never a verdict that they're harmful", "'They cause disease'"], correct: 1, why: "Mainstream institutional evidence (Johns Hopkins, Mass General, AHA) does not support 'seed oils are harmful.' State the choice, never the verdict." },
    ],
  },
  {
    slug: "the-craft", section: "brand", title: "The Craft — the process story, told straight", estMin: 10,
    summary: "How we make it — sourcing, cold extraction, glass & steel — and the exact line between a proud process story and a claim we can't make.",
    whyItMatters: "Guests and members feel the difference and want to know why. The craft is our edge — but the moment 'we take care' becomes 'so it's toxin-free / detoxes / is better for your genes,' we've traded a defensible story for a warning letter. This lesson lets you tell the whole story and stay bulletproof.",
    objectives: ["Tell the four-step process — sourced, drawn cold, poured into glass, made to order", "Explain the materials story (glass & stainless are inert) in the safe register", "Never cross into 'toxin-free,' 'detox,' 'low-acid,' or 'gene expression'"],
    founderInsight: "Coffee and cocoa are on the be-careful list for a reason — and that's exactly why the care matters. We keep the good by doing the work most skip: origin sourcing, cold extraction, inert glass and steel at every point it touches. Tell them what we DO. Never promise what it does to their body — the process is the flex, not a health claim.",
    body: [
      { h: "Keep the good, add nothing", p: "Coffee and cocoa carry plant-defense compounds — that's real, not marketing. We answer it with care: careful origin sourcing, testing, low-heat extraction, inert materials. Frame it as care, never cure. Never say we make coffee 'carnivore-safe,' 'antinutrient-free,' or that we 'remove plant toxins' — those compounds are inherent; we minimize the concerns, we don't erase them." },
      { h: "Sourced by origin + tested", p: "A plant draws what its soil and drying give it, so origin is the first lever. We source and test to keep mycotoxins (coffee) and heavy metals (cocoa) low and within accepted limits. Say 'tested-low, within limits.' NEVER 'toxin-free,' 'mold-free,' or 'heavy-metal-free' — those are measured in micrograms, not proven absent." },
      { h: "Drawn cold", p: "Cold-extracted, steeped slow in cool water, never rushed with heat — smoother and less bitter. That's a taste story. Never 'low-acid' or 'easier on your stomach' — the pH is comparable to hot brew; keep it sensory." },
      { h: "Poured into glass", p: "Glass and food-grade stainless at every point the coffee touches — inert, so no BPA, BPS, or plastic plasticizers migrate in. This is a MATERIALS statement — the safe register: what we don't introduce, never a health verdict on the drinker. And we vet the caps and seals too, not just the bottle — glass earns the win, closures have to earn it too." },
      { h: "Made the moment you order", p: "Nothing sits, nothing's preserved into shelf-life. Fresh because it's built in front of you — the last, unfakeable step." },
      { h: "The molecule = GT3", p: "Caffeine is a purine with three methyl groups at its nearest points — three methyls, three 3s, built into the chemistry. Fair to describe as brand identity and factual structure. Don't attach a health benefit to the molecule beyond 'caffeine is a stimulant.'" },
      { h: "The one line we never cross", p: "Never say the coffee, materials, or process 'positively affects your gene expression,' is 'net-positive on your genes,' or 'detoxes' you. That's the single highest-liability claim — human evidence is preliminary and it edges into a drug claim. Reframe to what we DON'T introduce (the plasticizers/microplastics of plastic) and general 'natural source of polyphenol antioxidants' language." },
    ],
    mistakes: ["Saying 'toxin-free,' 'non-toxic,' 'mold-free,' or 'heavy-metal-free' — claim tested-low/within-limits instead", "Calling cold brew 'low-acid' or 'easier on your stomach' — keep it 'smoother, less bitter'", "Any 'better for your genes / gene expression / detox' claim — retired, highest liability", "Saying 'zero microplastics' because it's glass — the caps matter; say 'we vet every contact surface'", "Promising the process makes coffee 'carnivore-safe' or removes plant compounds"],
    quiz: [
      { q: "A member asks: 'So your process makes the coffee toxin-free?'", options: ["'Yes, completely toxin-free'", "'We source and test to keep it clean and within limits — tested-low, not a claim of zero, because that's the honest way to say it'", "'Yes, no mold, no metals, nothing'"], correct: 1, why: "Mycotoxins and heavy metals are measured in micrograms, not proven absent. Tested-low/within-limits is the defensible claim; absolutes aren't." },
      { q: "Why do we pour into glass and stainless — how do you say it?", options: ["'Glass detoxifies the coffee'", "'They're inert, so no BPA, BPS, or plasticizers migrate in — we control what's in the bottle by controlling what it touches'", "'Glass makes it better for your genes'"], correct: 1, why: "Materials/composition statement (what we don't introduce) is the safe register. Never a gene or detox claim." },
      { q: "A guest says cold brew is 'low-acid, easier on the stomach.' You…", options: ["Agree — it's much less acidic", "Say 'many find it smoother and less bitter; the pH is actually similar to hot brew, so I'd keep it to taste'", "Say it cures acid reflux"], correct: 1, why: "Cold brew's measured pH is comparable to hot brew; the honest claim is sensory (smoother/less bitter), not chemistry or digestion." },
      { q: "Can we say our coffee 'positively affects your gene expression'?", options: ["Yes — there's research on polyphenols and epigenetics", "No — it's the highest-liability claim; human evidence is preliminary and it edges into a drug claim. Reframe to what we don't introduce", "Yes, if we cite a study"], correct: 1, why: "The epigenetics research is early, mostly-laboratory, and correlational. As worded it's an unsubstantiated structure/function claim. Retire it." },
      { q: "Best way to tell the 'every point it touches' story?", options: ["'Glass means zero microplastics, guaranteed'", "'Glass is inert, and we vet the caps and seals too — glass earns the win, the closures have to earn it too'", "'Plastic gives you cancer, glass doesn't'"], correct: 1, why: "Glass isn't automatically microplastic-free (caps can shed), so the honest, stronger story is diligence at every surface — never an absolute, never a health verdict on the drinker." },
    ],
  },
  {
    ...
    body: [
      ...
      { h: "Mission", p: "Make the most honest beverages we can — every input a named whole food, every claim something we can stand behind — and serve them at the moments people actually need them." },
