...
export function lintCaption(text: string): LintFinding[] {
  ...
  if (/\b(no|zero|without)\b[^.!?\n]*\bsugar\b/i.test(t) || /\bsugar[- ]?free\b/i.test(t)) out.push({ level: "warn", tag: "disclosure", msg: "Says no/zero sugar — disclose the sweetener (organic local honey in Tide, organic maple in Nature Aide / Salted Maple, and other sweetened items)." });
