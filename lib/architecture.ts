...
export const DATABASES: DbEntry[] = [
  ...
  { table: "reserves", manage: "partial", surface: "Money → Reserve drops", note: "Create + update." },
...
export const BUSINESS: BizCapability[] = [
  ...
  },
  {
    ...
    built: [
      "Company Calendar — events + admin/ops + to-dos, rolled up by work stream, click-through to source",
    ...
    where: "Plan → Calendar · Business → Notes",
  ...
  },
  {
    ...
    built: [
      "Role-scoped crew console at /crew — pinnable work streams (Today · Service · Events · Production · Brand · Business)",
  ...
  },
  {
    ...
    built: [
      ...
      "Reserve drops (limited releases) — created and tracked from Money",
    ],
    where: "Money",
  },
  {
...
export const ARCHITECTURE: ArchLayer[] = [
  ...
  },
  {
    ...
    components: [
      ...
      { name: "Web search tool", status: "configured", desc: "Server-side web_search for the inspection agent.", detail: "One focused search (max_uses:1 + at most one resume) in the background research job — the agentic search is the dominant cost, so it's kept lean to stay under the 120s function cap.", config: "Enable web search for the workspace in the Anthropic Console", sot: "app/api/agents/inspection/route.ts" },
