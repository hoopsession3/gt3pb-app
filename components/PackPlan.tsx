...
export default function PackPlan({ ownerType, ownerId, title, onClose }: { ownerType: "event" | "stop"; ownerId: string; title: string; onClose: () => void }) {
  ...
  const [fleet, setFleet] = useState<{ cap: number; qty: number }[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setFleet(((k as { capacity_gal: number; qty: number }[]) ?? []).map((x) => ({ cap: Number(x.capacity_gal), qty: Number(x.qty) })).filter((x) => x.cap > 0 && x.qty > 0));
    setLoaded(true);
  }, [ownerCol, ownerId]);
  useEffect(() => { load(); }, [load]);
