...
// RPC funnel_counts(); the raw rows are never exposed to the client.
...
export default function FunnelReport() {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase.rpc("funnel_counts", { p_days: days });
    setRows((data as Row[]) ?? []);
  }, [days]);
  useEffect(() => { load(); }, [load]);
  ...
  if (rows === null) return <div className="fn-empty">Loading…</div>;
  ...
  return (
    <div className="fn">
      ...
      {!anyData ? (
        <div className="fn-empty">No funnel activity yet in this window — it fills in as guests move through order, reserve, delivery and sign-up.</div>
      ) : FUNNELS.map((f) => {
