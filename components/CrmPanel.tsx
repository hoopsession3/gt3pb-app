...
function CrmDetail({ c }: { c: Customer }) {
  ...
  return (
    <AsyncSection state={board} isEmpty={() => false} loadingLabel="Loading history…" errorTitle="Couldn't load this customer's history" emptyTitle="Nothing here yet">
      {() => (
    ...
    <AsyncSection state={board} isEmpty={(data) => data.length === 0} emptyTitle="No customers yet" emptySub="They appear with their first order." errorTitle="Couldn't load customers">
      {() => (
        <div className="adm-sec">
          ...
          {rows.length > 0 && shown.length === 0 && <div className="h-sub">No match for &ldquo;{q}&rdquo;.</div>}
        </div>
      )}
    </AsyncSection>
  );
