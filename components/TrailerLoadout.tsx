...
// Fetch state via useAsyncData — before, a failed trailer_profile fetch left `tp` null and the WHOLE
// component silently rendered nothing (not even an empty state) — now a failed load is a real error.
type EvRow = { id: string; title: string; is_live?: boolean; day?: string | null };
type StopRow = { id: string; name: string; starts_at?: string | null };
type Board = { tp: TrailerProfile | null; assets: AssetDim[]; events: EvRow[]; stops: StopRow[] };
...
export default function TrailerLoadout({ lockTo }: { lockTo?: { kind: "event" | "stop"; id: string } } = {}) {
  ...
  const loader = useCallback(async (): Promise<Board> => {
    if (!supabase) return { tp: null, assets: [], events: [], stops: [] };
    const [t, a] = await Promise.all([
      supabase.from("trailer_profile").select("*").eq("id", 1).maybeSingle(),
      supabase.from("assets").select("name, len_in, width_in, height_in").not("len_in", "is", null),
    ]);
    if (t.error) throw new Error(t.error.message);
    if (a.error) throw new Error(a.error.message);
    const tp = (t.data as TrailerProfile) ?? null;
    const assets = (a.data as AssetDim[]) ?? [];
    if (lockTo) return { tp, assets, events: [], stops: [] };
    ...
    return { tp, assets, events: (e.data as EvRow[]) ?? [], stops: (s.data as StopRow[]) ?? [] };
  ...
  // Embedded in a hub → scope to that one owner, no picker. Otherwise resolve a default target
  // once the board loads: keep the current pick if it's still valid, else live event, else next
  // upcoming, else the first thing on the books.
  useEffect(() => {
    if (lockTo) { setSel(`${lockTo.kind === "stop" ? "s" : "e"}:${lockTo.id}`); return; }
  ...
  }, [board.data, lockTo, events, stops]);

  const targets = useMemo(() => lockTo ? [] : [
    ...events.map((e) => ({ key: `e:${e.id}`, label: `🎪 ${e.title}` })),
    ...stops.map((s) => ({ key: `s:${s.id}`, label: `🚚 ${s.name}` })),
  ], [events, stops, lockTo]);

  // Pack labels + rig for the SELECTED event/stop drive the loadout weight + space. Separate,
  // sel-scoped fetch — left as its own concern (a miss here degrades to an empty pack list, not a
  // vanished page), same as the original.
  useEffect(() => {
  ...
  useRealtimeTable(["trailer_profile", "event_tasks", "stops"], reload);
  ...
  const saveProfile = async () => {
    ...
    toast("Trailer profile saved"); setEdit(false); setForm({}); reload();
  };
  ...
  return (
    <AsyncSection state={board} isEmpty={(data) => data.tp === null} emptyTitle="No trailer profile yet" emptySub="Set one up to see weight and space checks." errorTitle="Couldn't load the load-out">
      {(data) => {
        const tp = data.tp;
        if (!tp) return null;
        ...
        return (
          <div className="adm-sec tl">
            <SectionHeader label="Load-out & tow plan" />
