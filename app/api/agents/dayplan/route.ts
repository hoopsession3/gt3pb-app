...
export async function POST(req: Request) {
  ...
  if (body.summarize) {
    const ev2 = eventId
      ? (await supabaseAdmin.from("events").select("title, day_label, location_text").eq("tenant_id", tenant).eq("id", eventId).maybeSingle()).data
  ...
  try {
    const r = await callClaude({ label: "dayplan",
      ...
      temperature: 0.3,
      system:
        "You plan logistics for GT3 Performance Bar, a mobile beverage truck run by a small owner-led crew. " +
