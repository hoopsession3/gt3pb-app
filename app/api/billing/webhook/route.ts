...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (evt.type === "checkout.session.completed" && o.client_reference_id) {
      ...
      await raiseAlert({ severity: "important", category: "billing", title: "New software subscription", body: `Tenant ${o.client_reference_id} subscribed.`, link: "/admin" });
    }
    ...
    if ((evt.type === "customer.subscription.updated" || evt.type === "customer.subscription.deleted") && o.id) {
      ...
      if (status === "past_due" || status === "canceled") {
        await raiseAlert({ severity: "important", category: "billing", title: `Software subscription ${status}`, body: `Stripe subscription ${o.id} is ${status}.`, link: "/admin" });
      }
    }
