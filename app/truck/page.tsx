...
import RouteMap, { type RoutePoint, type TruckPos } from "@/components/RouteMap";
...
// Counts down to a target time. Returns null when there's no known target (so the UI can
// hide the pill rather than invent a clock). The target is a timestamp the caller memoizes.
function useCountdown(target: number | null) {
  const [cd, setCd] = useState<string | null>(null);
  useEffect(() => {
    if (target == null) { setCd(null); return; }
    const tick = () => {
...
// Resolve a countdown target (ms) from a stop's starts_at, falling back to a "HH:MM" eta
// interpreted as the next occurrence of that time. Returns null when nothing is known.
function targetFrom(startsAt: string | null | undefined, eta: string | null | undefined): number | null {
  if (startsAt) {
    const d = new Date(startsAt).getTime();
    if (!Number.isNaN(d) && d > Date.now()) return d;
  }
  const m = eta ? /^(\d{1,2}):(\d{2})$/.exec(eta.trim()) : null;
  if (m) {
    const d = new Date();
...
// Next occurrence of a wall-clock time today/tomorrow — used only by the static demo.
function nextOccurrence(h: number, min: number): number {
  const d = new Date();
...
function TruckLive() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "live_status" }, (p) => {
        // A DELETE delivers an empty `new` — fall back to a reload instead of blanking state.
  ...
  // Single source of truth: a stop is "live" only when the truck is live AND it's the current stop.
  const liveStopId = isLive ? live?.current_stop_id ?? null : null;

  const currentStop = stops.find((s) => s.id === live?.current_stop_id);
  const nextStop = stops.find((s) => s.status !== "done" && s.id !== live?.current_stop_id) ?? stops.find((s) => s.status !== "done");
  // When live, headline the current stop; when paused/offline, headline the next stop to visit.
  const liveStop = isLive ? currentStop ?? stops[0] : nextStop ?? currentStop ?? stops[0];

  const target = useMemo(() => targetFrom(nextStop?.starts_at, live?.next_eta), [nextStop?.starts_at, live?.next_eta]);
  const cd = useCountdown(target);

  const truck: TruckPos | null =
  ...
  const points = useMemo<RoutePoint[]>(
    () =>
      stops
        ...
        .map((s) => ({ name: s.name, lat: s.lat as number, lng: s.lng as number, live: s.id === liveStopId })),
    [stops, liveStopId]
  );
    ...
    <section className="screen" id="s-truck">
      ...
      <div className="hero"><div className="hin">
        ...
        <div className="cells">
          ...
          <div className="cell"><div className="cv ok">{live?.next_eta ?? "—"}</div><div className="cl">ETA</div></div>
        </div>
        {cd && <div className="countpill"><span className="cl">Next stop in</span><span className="cd">{cd}</span></div>}
      ...
      {points.length >= 2 && (
        <>
          ...
          <RouteMap points={points} truck={truck} />
        </>
      )}

      <div className="sec">This week</div>
      {stops.map((s) => {
        const rowLive = s.id === liveStopId;
        return (
          <div
            key={s.id}
...
function TruckDemo() {
  const { toast } = useApp();
  const router = useRouter();
  const cd = useCountdown(useMemo(() => nextOccurrence(16, 30), []));
  return (
    <section className="screen" id="s-truck">
      <div className="toprow">
        <div className="eyb">On the ground</div>
